...
const BlogDetailView = ({ blogId, onBack }) => {
  const { user } = useAuth();
  ...
  const loadBlogDetail = async () => {
    try {
      ...
      setBlog(data.blog);
      setLikeCount(Array.isArray(data.blog.likes) ? data.blog.likes.length : data.blog.likes || 0);
      if (user && Array.isArray(data.blog.likes)) {
        setLiked(data.blog.likes.includes(user.id));
      }
    } catch (error) {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      await api.addComment(blog._id, commentText);
      setCommentText('');
      loadBlogDetail(); // Refresh to get updated comments
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <ModernSkeleton className="h-96 p-6" lines={15} />
      </div>
    );
  }
  ...
  const isAuthor = user && blog.author && (user.id === blog.author._id || user.id === blog.author.id);
        ...
        {blog.featuredImage && (
          <div className="relative h-64 md:h-80 overflow-hidden">
            ...
            <div className="absolute bottom-4 left-6">
              <ModernBadge variant="glass" size="md">
                {blog.category}
              </ModernBadge>
            </div>
          </div>
        ...
        <div className="p-8">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex-1">
              <h1 className="text-3xl md:text-4xl font-bold text-dark-900 mb-4 leading-tight">
                {blog.title}
              </h1>
              ...
              <div className="flex items-center text-sm text-dark-500 mb-4">
                ...
                <span className="font-medium mr-2">{blog.author?.username || 'Unknown'}</span>
            ...
            {(isAuthor || isAdmin) && (
              <div className="flex items-center space-x-3">
                {isAuthor && ['pending', 'rejected', 'draft'].includes(blog.status) && (
                  <ModernButton
                    variant="secondary"
                    size="sm"
                    icon={Edit}
          ...
          {blog.tags && blog.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-6">
              {blog.tags.map((tag, index) => (
                <ModernBadge key={index} variant="glass" size="sm">
                  #{tag}
                </ModernBadge>
              ))}
            </div>
          ...
          <div className="prose prose-lg max-w-none mb-8">
            <div className="text-dark-700 leading-relaxed whitespace-pre-wrap">
              {blog.content}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between py-6 border-t border-gray-200">
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-2 text-dark-500">
                ...
                <span className="font-medium">{Array.isArray(blog.comments) ? blog.comments.length : 0} comments</span>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 pt-8">
            <h3 className="text-xl font-bold text-dark-900 mb-6">Comments</h3>
                ...
                blog.comments.map((comment, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-700 leading-relaxed">{comment.text || comment.content}</p>
                    </div>
                  </motion.div>
                ))
              ) : (
...
// Blog Card Component
const BlogCard = ({ blog, onLike, onComment, onRefresh, onViewFull, onDelete }) => {
  const { user } = useAuth();
  const [liked, setLiked] = useState(false);
  ...
  const handleViewFull = (e) => {
    e.stopPropagation(); // Prevent card click when View Full button is clicked
    onViewFull && onViewFull(blog._id);
  };
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            {canDelete && (
              <motion.button
                onClick={handleDelete}
                className="flex items-center space-x-1 px-3 py-2 rounded-lg text-red-500 hover:text-red-700 hover:bg-red-50 transition-all duration-300"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                title="Delete blog"
              >
...
// Home Page Component
const HomePage = ({ onViewBlogDetail }) => {
...
const TrendingPage = ({ onViewBlogDetail }) => {
...
const App = () => {
  ...
  const renderCurrentView = () => {
    if (!user && currentView !== 'home' && currentView !== 'trending' && currentView !== 'blog-detail') {
      return <LoginForm onSuccess={handleLoginSuccess} />;
    }

    switch (currentView) {
      case 'home':
        return <HomePage onViewBlogDetail={handleViewBlogDetail} />;
      case 'trending':
        return <TrendingPage onViewBlogDetail={handleViewBlogDetail} />;
      case 'blog-detail':
        return <BlogDetailView blogId={selectedBlogId} onBack={handleBackFromDetail} />;
      case 'create':
